...
- Live App: https://project-5101d58b-03f9-4d7e-a71.web.app/
- Backend API: https://maapahdeed-8597.onrender.com/api
...
| API Integration | The backend calls external APIs including E-Chords for song chords, All Guitar Chords for guitar-shape lookup, Chords API for chord analysis fallback, iTunes Search API for artwork, and MusicBrainz for recording search. API responses are used directly in Battle, the fretboard guide, and Song Chord Quest. |
...
- APIs / Libraries: E-Chords song chord lookup, All Guitar Chords shape lookup, Chords API fallback, iTunes Search API artwork lookup, MusicBrainz recording search, MediaPipe Hands, MediaPipe Face Mesh, MediaPipe Camera Utils, Web Audio API, Pitchy
...
Keep `backend/.env` private. Use `backend/.env.example` as the shared template.

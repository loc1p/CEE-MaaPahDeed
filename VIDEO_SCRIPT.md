...
ลิงก์ live ของโปรเจกต์คือ `https://maapahdeed.onrender.com/` และ backend API อยู่ที่ `https://maapahdeed.onrender.com/api`

โชว์:

- แถบ address bar ที่เป็น public URL
- เปิด `https://maapahdeed.onrender.com/api/health` สั้น ๆ เพื่อโชว์ว่า backend ทำงาน
- Refresh หน้าเว็บ
...
โปรเจกต์นี้เตรียม deploy ผ่าน Render โดยมีไฟล์ `render.yaml` ให้ Render อ่านเป็น Blueprint และใช้ MongoDB Atlas เป็น database สำหรับระบบ login, saved songs และ dashboard
...
Battle mode ช่วยให้การฝึกคอร์ดมีความเป็นเกมมากขึ้น ผู้ใช้ไม่ได้แค่จำคอร์ดจากตาราง แต่ได้ฝึกผ่าน target, feedback และ progress ในเกม เมื่อผู้ใช้เล่นคอร์ดถูก แอปจะนับ hit และแสดงคอร์ดนั้นค้างไว้ในช่อง Chord จนกว่าจะเล่นถูกคอร์ดใหม่
